...
const validator = (0, util_1.safeRequire)('class-validator', () => require('class-validator'));
const transformer = (0, util_1.safeRequire)('class-transformer', () => require('class-transformer'));
class BulkDto {
}
class Validation {
    static getValidationPipe(options, group) {
        return validator && !(0, crud_util_1.isFalse)(options.validation)
    ...
    static createBulkDto(options) {
        if (validator && transformer && !(0, crud_util_1.isFalse)(options.validation)) {
            const { IsArray, ArrayNotEmpty, ValidateNested } = validator;
            const { Type } = transformer;
            const hasDto = !(0, crud_util_1.isNil)(options.dto.create);
