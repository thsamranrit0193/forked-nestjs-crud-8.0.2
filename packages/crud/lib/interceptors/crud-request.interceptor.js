...
let CrudRequestInterceptor = class CrudRequestInterceptor extends crud_base_interceptor_1.CrudBaseInterceptor {
    intercept(context, next) {
        ...
        try {
            if (!req[constants_1.PARSED_CRUD_REQUEST_KEY]) {
                ...
                if (!(0, crud_util_1.isNil)(ctrlOptions)) {
    ...
    getSearch(parser, crudOptions, action, params) {
        ...
        if ((0, crud_util_1.isFunction)(crudOptions.query.filter)) {
            const filterCond = crudOptions.query.filter(parser.search, action === enums_1.CrudActions.ReadAll) || {};
            return [...paramsSearch, filterCond];
        }
        const optionsFilter = (0, crud_util_1.isArrayFull)(crudOptions.query.filter)
        ...
        else if ((0, crud_util_1.hasLength)(parser.filter) && (0, crud_util_1.hasLength)(parser.or)) {
            search =
        ...
        else if ((0, crud_util_1.hasLength)(parser.filter)) {
            search = parser.filter.map(parser.convertFilterToSearch);
        }
        else {
            if ((0, crud_util_1.hasLength)(parser.or)) {
                search =
                    parser.or.length === 1
                        ? [parser.convertFilterToSearch(parser.or[0])]
                        : [
    ...
    getParamsSearch(parser, crudOptions, params) {
        if (params) {
            ...
            return (0, crud_util_1.isArrayFull)(parser.paramsFilter)
    ...
    getAuth(parser, crudOptions, req) {
        ...
        if (crudOptions.auth) {
            const userOrRequest = crudOptions.auth.property
                ...
                : req;
            if ((0, crud_util_1.isFunction)(crudOptions.auth.or)) {
                auth.or = crudOptions.auth.or(userOrRequest);
            }
            if ((0, crud_util_1.isFunction)(crudOptions.auth.filter) && !auth.or) {
                auth.filter =
                    crudOptions.auth.filter(userOrRequest) || {};
            }
            if ((0, crud_util_1.isFunction)(crudOptions.auth.persist)) {
...
exports.CrudRequestInterceptor = CrudRequestInterceptor = __decorate([
